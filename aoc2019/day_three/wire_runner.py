...
"""
This module provides the logic for day 3 aoc2019 solution with utility
...
def init_grid(size):
    ...
    odd_size = size + (size + 1) % 2
    return [[Empty() for j in range(odd_size)] for i in range(odd_size)]


def resize_grid(grid):
    """
    Utility function to make a new grid double in size and transfer existing
    entities.
    """
...
def get_intersections(grid):
    """
    This utility function finds all the intersections in a grid. It returns a
    list of (x, y) tuples.
    """
...
class WireGrid:
    """
    Class to represent a grid with Wires mapped on it. Provides attributes and
    behaviors to map "wires" onto the grid based a list of "tracing"
    instructions. Works when grid is small. Becomes unmanageable with large
    grid sizes.
    """
    ...
    def get_min_manhattan_dist(self):
        """
        This method returns the manhattan distance from closest intersection
        from the origin.
        """
    ...
    def trace_wires(self, trace):
        """
        Method maps a series of passed "trace" instructions to the grid.
        Expected instructions are in the form of D123 to go down 123 cells
        or R03 to go right 3 cells.
        """
    ...
    def up(self, dist):
        ...
        for offset in range(1, dist + 1):
            try:
                current = self.grid[self.current_pos['y']
                                    - offset][self.current_pos['x']]
                # dynamically dispatch action
                self.grid[self.current_pos['y']
                          - offset][self.current_pos['x']] = \
...
class WireTable:
    ...
    def __init__(self):
        ...
        # list of dictionaries of steps per intersection
        self.list_keyed_intersect_steps = None
        ...
        self.instructions = {'U': self.up,
                             ...
                             'L': self.left}
        # Map instruction codes to helper methods
        self.count_steps_instr = {'U': self.up_count,
                                  'D': self.down_count,
                                  'R': self.right_count,
                                  'L': self.left_count}
    ...
    def trace_wires(self, trace):
        """
        Method maps a series of passed "trace" instructions to the trace
        dictionaries. Expected instructions are in the form of D123 to go
        down 123 cells or R03 to go right 3 cells.
        """
    ...
    def find_intersections(self):
        """
        Helper method that iteratively saves list of intersections for both
        traces.
        """
        self.intersections = []
        # iterate through
    ...
    def find_min_steps(self):
        """
        Find the minimum number of steps required to reach an intersection.
        """
        ...
        for cross in self.intersections:
            steps = self.list_keyed_intersect_steps[0][cross] \
                    + self.list_keyed_intersect_steps[1][cross]
